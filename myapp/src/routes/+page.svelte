<script>
    ...
    async function remindRole() {
        // Make the API Call here
		console.log(rowdict[playerDict["Player"]]);
    }
	...
	function getResult(){
		endStr = "THE END;";
		endStr += president + " is the President;";
		endStr += assassin + " is the Assassin;";
		...
		if (president == "Player"){
			...
			roomPresident = otherroom;
			for (var i = 0; i < 3; i++){
				if (arrayOfNames[i] == president){
					roomPresident = room;
				}
			}
		}
		...
		if (assassin == "Player"){
			...
			roomAssassin = otherroom;
			for (var i = 0; i < 3; i++){
				if (arrayOfNames[i] == assassin){
					roomAssassin = room;
				}
			}
		}

		if (roomAssassin == roomPresident){
			endStr += "RED TEAM WON";
		} else {
			endStr += "BLUE TEAM WON"
		}
	}
	...
	function toggleState(newState){
		if (state == 0){
			// role = getRandomInt(8);
			// playerDict["Player"] = role
			
			arrayOfNames = shuffle(arrayOfNames);
			const arrayOfRoles = shuffle([0, 1, 2, 3, 4, 5, 6, 7]);
			president = "Player";
			assassin = "Player";
			for (var i = 0; i < 7; i++){
				playerDict[arrayOfNames[i]] = arrayOfRoles[i];
				if (arrayOfRoles[i] == 0){
					president = arrayOfNames[i];
				} else if (arrayOfRoles[i] == 4){
					assassin = arrayOfNames[i];
				}
			}
			playerDict["Player"] = arrayOfRoles[7];
			aha1 = arrayOfNames[0];
			aha2 = arrayOfNames[1];
			aha3 = arrayOfNames[2];
			console.log("President is ", president)
			console.log("Assassin is ", assassin)
...
<section>
	...
	{#if state == 4}
		...
		<button on:click={()=>{if (round < 3){toggleState(1);} else {getResult();toggleState(5);}}}>Ok, noted.</button>
	{:else if state == 5}
		<ResultPage eventStr={endStr}/>
		...
		{:else}
			...
			{#if state == 3}
				<div>
					<div>
						<h3>
						Round: {round} &nbsp&nbsp&nbsp&nbsp&nbsp Room: {room}
						<br>
				...
				<div>
				<CardSpecial role={rowdict[playerDict["Player"]]} name={"Player"} />
				</div>
				</div>
			...
			{:else}
				...
				{:else}
					<div>
						...
						<div>
						<CardSpecial role={rowdict[playerDict["Player"]]} name={"You"} />
						</div>
					</div>
